...
async function viewEmployeesByRole() {
  ...
  const roleRecord = roles.find((resultEntry) => resultEntry.title === answer.role);
...
async function addEmployee() {
  ...
  if (answer.manager !== "None") {
    const managerRecord = employees.find(
      (resultEntry) => answer.manager === resultEntry.first_name + " " + resultEntry.last_name
    );
...
async function deleteEmployee() {
  ...
  const employeeChoice = employees.find((resultEntry) => {
    return answer.employee === resultEntry.first_name + " " + resultEntry.last_name;
  });
